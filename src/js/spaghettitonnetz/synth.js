...
class Synth {
    ...
    playMidiNote(note) {
        if (!this.enabled) return;
        this.polySynth.playMidiNote(note);
    }
    ...
    releaseMidiNote(note) {
        if (!this.enabled) return;
        this.polySynth.releaseMidiNote(note);
    }
    ...
    setPitchBend(value) {
        if (!this.enabled) return;
        this.polySynth.setPitchBend(value);
    }
...
class MonoSynth {
    ...
    playMidiNote(midiNote){
        ...
        this.playNote(Math.pow(2,((this.midiNote+this.bend)-69)/12.0)*440);
    }
    ...
    setPitchBend(value) {
        this.bend = value;
        this.frequency = Math.pow(2,((this.midiNote+this.bend)-69)/12.0)*440;
        this.osc.frequency.linearRampToValueAtTime(this.frequency,this.audioContext.currentTime);
    }
...
class PolySynth {
    ...
    playMidiNote(note) {
        ...
        if (number >= 0) this.playMidiNoteOnSynth(number,note);
    }
    ...
    playMidiNoteOnSynth(number, note) { 
        this.state[number][1] = true;
        this.state[number][2] = note;
        this.state[number][3] = this.audioContext.currentTime;
        this.state[number][0].playMidiNote(note);
    }
    ...
    releaseMidiNote(note) {
        for (let i = 0; i < this.voices; i++) {
            if (this.state[i][1] && this.state[i][2] === note){
                this.state[i][1] = false;
                this.state[i][0].releaseNote();
                this.state[i][4] = this.audioContext.currentTime;
            }
    ...
    setPitchBend(value) {
        this.bend = value;
        for (let i = 0; i < this.voices; i++) this.synths[i].setPitchBend(value);
    }
