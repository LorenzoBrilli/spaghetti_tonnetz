...
class Synth {
    ...
    enable() {
        if (this.audioContext === undefined){
            ...
            this.polySynth.setWave("triangle");
            this.polySynth.setVolume(0.2);
...
class MonoSynth {
    ...
    constructor (audioContext) {
        ...
        this.filter.frequency.value = 40000;
    ...
    playNote(freq) {
        ...
        this.adsrGain.gain.linearRampToValueAtTime(this.adsr[2],this.audioContext.currentTime+(this.adsr[0]+this.adsr[1])/1000);  
    }
    releaseNote() {
        this.adsrGain.gain.cancelScheduledValues(this.audioContext.currentTime);
...
class PolySynth {
    ...
    setAdsr(attack,delay,sustain,release) {
        ...
        this.adsr[2] = sustain;
        this.adsr[3] = release;
        for (let i = 0; i < this.voices; i++) this.synths[i].setAdsr(attack,delay,sustain,release);
    }
